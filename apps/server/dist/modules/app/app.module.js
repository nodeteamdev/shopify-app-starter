"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppModule = void 0;
const app_config_1 = require("../../config/app.config");
const jwt_config_1 = require("../../config/jwt.config");
const mailer_config_1 = require("../../config/mailer.config");
const ngrok_config_1 = require("../../config/ngrok.config");
const redis_config_1 = require("../../config/redis.config");
const sentry_config_1 = require("../../config/sentry.config");
const shopify_config_1 = require("../../config/shopify.config");
const swagger_config_1 = require("../../config/swagger.config");
const app_controller_1 = require("./app.controller");
const app_service_1 = require("./app.service");
const auth_module_1 = require("../auth/auth.module");
const email_service_1 = require("../email/email.service");
const mandatory_webhook_module_1 = require("../mandatory-webhook/mandatory-webhook.module");
const shopify_app_install_module_1 = require("../shopify-app-install/shopify-app-install.module");
const shopify_auth_module_1 = require("../shopify-auth/shopify-auth.module");
const user_module_1 = require("../user/user.module");
const webhook_module_1 = require("../webhook/webhook.module");
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const jwt_1 = require("@nestjs/jwt");
const throttler_1 = require("@nestjs/throttler");
const prisma_1 = require("../common/providers/prisma");
const redis_throttler_storage_service_1 = require("../common/providers/redis/redis-throttler-storage.service");
const redis_module_1 = require("../common/providers/redis/redis.module");
const nestjs_redis_1 = require("@songkeys/nestjs-redis");
const logger = new common_1.Logger('AppModule');
let AppModule = class AppModule {
};
exports.AppModule = AppModule;
exports.AppModule = AppModule = __decorate([
    (0, common_1.Module)({
        imports: [
            config_1.ConfigModule.forRoot({
                isGlobal: true,
                load: [
                    app_config_1.appConfig,
                    sentry_config_1.sentryConfig,
                    redis_config_1.redisConfig,
                    ngrok_config_1.ngrokConfig,
                    swagger_config_1.swaggerConfig,
                    mailer_config_1.mailerConfig,
                    jwt_config_1.jwtConfig,
                    shopify_config_1.shopifyConfig,
                ],
            }),
            prisma_1.PrismaModule,
            nestjs_redis_1.RedisModule.forRootAsync({
                useFactory: async (configService) => {
                    const redisConfig = configService.get('redis');
                    const redisUrl = new URL(redisConfig.url);
                    return {
                        config: {
                            url: redisUrl.toString(),
                            onClientCreated: (client) => {
                                client.on('ready', () => logger.log(`Redis is started on port ${redisUrl.port}`));
                                client.on('error', (error) => {
                                    logger.error(error);
                                });
                                client.on('restart', () => logger.warn('Attempt to restart the redis server'));
                            },
                        },
                    };
                },
                inject: [config_1.ConfigService],
            }),
            auth_module_1.AuthModule,
            throttler_1.ThrottlerModule.forRootAsync({
                useFactory: (redis, configService) => {
                    const appConfig = configService.get('app');
                    const ttl = appConfig.rateLimitTtl;
                    const limit = appConfig.rateLimitTimes;
                    return {
                        throttlers: [{ ttl, limit }],
                        storage: redis,
                    };
                },
                inject: [redis_throttler_storage_service_1.RedisThrottlerStorageService, config_1.ConfigService],
                imports: [redis_module_1.RedisModule, config_1.ConfigModule],
            }),
            jwt_1.JwtModule.register({
                global: true,
            }),
            user_module_1.UserModule,
            webhook_module_1.WebhookModule,
            shopify_app_install_module_1.ShopifyAppInstallModule,
            mandatory_webhook_module_1.MandatoryWebhookModule,
            shopify_auth_module_1.ShopifyAuthModule,
        ],
        controllers: [app_controller_1.AppController],
        providers: [app_service_1.AppService, email_service_1.EmailService],
    })
], AppModule);
//# sourceMappingURL=app.module.js.map